...
def list_files(path: str, include_subfolder: bool = False, name_only: bool = False, strict_exist: bool = False, ext: str = '') -> list:
    ...
    try:
        ...
        for item in rawlst:
            fullpath = str(Path(path).joinpath(item))

            if os.path.isdir(fullpath) and include_subfolder:
                endlist.extend([
                    filename if name_only else str(Path(item).joinpath(filename))
                ...
                endlist.append(item if name_only else fullpath)

        return [f for f in endlist if not ext or f.endswith(ext)]
