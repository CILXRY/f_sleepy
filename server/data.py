...
class Data:
    ...
    def _init_main_data(self):
        '''初始化主数据记录'''
        if not _MainData.query.first():
            l.debug(f'[data] main_data not exist, creating a new one')
            db.session.add(_MainData())
            db.session.commit()

    def _init_metrics_metadata(self):
        '''初始化统计元数据记录'''
        if not _MetricsMetaData.query.first():
            l.debug(f'[data] metrics_metadata not exist, creating a new one')
            db.session.add(_MetricsMetaData())
            db.session.commit()

    def _handle_db_error(self, e: SQLAlchemyError):
        '''
        统一处理数据库错误
        '''
        l.error(f'Database Error: {e}')
        raise u.APIUnsuccessful(500, 'Database operation failed')
        ...
        try:
            with self._app.app_context():
                ...
                if not device:
                    ...
                    device = _DeviceStatusData(id=id)
    ...
    def record_metrics(self, path: str, count: int = 1, override: bool = False) -> None:
        ...
        try:
            with self._app.app_context():
                metric = _MetricsData.query.filter_by(path=path).first()
                ...
                if override:
                    for field in ['daily', 'weekly', 'monthly', 'yearly', 'total']:
                        setattr(metric, field, count)
                else:
                    metric.daily += count
                    metric.weekly += count
    ...
    def _metrics_refresh(self):
        ...
        try:
            with self._app.app_context():
                raw_metrics = _MetricsData.query.all()
                meta_metrics = _MetricsMetaData.query.first()
